...
1. Document every schema version and the keys introduced, removed, renamed, or changed.
2. Never silently reinterpret an existing key. If a value's type or meaning changes, introduce a new versioned key.
3. Keep the old representation during migration: read, validate, convert, and write the new representation.
4. Make migrations explicit and repeatable, with a clear precondition, target version, validation checks, and completion state.
5. Do not delete old data until the new representation has been populated and verified.
...
2. Document the key and its value type.
3. Define the behavior when the key is absent.
...
existing values may fail to decode or may not contain the fields required by `NewStats`.
...
7. Mark the migration complete only after conversion succeeds.
...
Treat semantic changes like type changes: introduce a new representation, convert existing data deliberately, and document the conversion rule.
...
| `v3` | Changed project statistics representation | Read the previous representation, convert to the new representation, write versioned keys, verify, then mark `v3` complete | Requires migration |
