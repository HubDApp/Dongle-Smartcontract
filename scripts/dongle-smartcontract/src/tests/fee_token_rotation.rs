...
#[test]
fn test_admin_only_treasury_update() {
    let env = Env::default();
    env.mock_all_auths();
    let (client, admin, old_treasury, _token) = setup_with_token(&env);

    // Initial treasury check
    assert_eq!(client.get_treasury(), old_treasury);
...
fn test_treasury_updated_event_emitted() {
    ...
    let mut found_treasury_event = false;

    for event in events.iter() {
        if event.1 == (soroban_sdk::symbol_short!("CONFIG"), soroban_sdk::symbol_short!("TREASURY")).into_val(&env) {
            found_treasury_event = true;
            break;
        }
    }

    assert!(found_treasury_event, "TreasuryUpdated event should be emitted on set_treasury");
}
