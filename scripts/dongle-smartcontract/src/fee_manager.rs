...
use crate::events::{
    publish_fee_consumed_event, publish_fee_paid_event, publish_fee_set_event,
    publish_treasury_updated_event, FeeOperation,
};
...
impl FeeManager {
    ...
    pub fn set_fee(
        ...
        let old_treasury: Option<Address> = env.storage().persistent().get(&StorageKey::Treasury);
        ...
        if old_treasury.as_ref() != Some(&treasury) {
            publish_treasury_updated_event(
                env,
                admin.clone(),
                old_treasury,
                treasury.clone(),
            ...
            AdminActionLog::record_action(
                env,
                admin.clone(),
                AdminActionType::TreasuryUpdated,
                None,
                Some(treasury),
                None,
            );
    ...
    pub fn set_treasury(env: &Env, admin: Address, treasury: Address) -> Result<(), ContractError> {
        ...
        let old_treasury: Option<Address> = env
            .storage()
            .persistent()
            .get(&StorageKey::Treasury);
