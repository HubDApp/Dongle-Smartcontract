...
/// Storage keys for detected project fork and migration relationships (#748).
///
/// Fork relationships use a dedicated namespace so additions do not consume
/// the already-full `ExtensionKey` union. A child has at most one parent;
/// parents own a separate child index for bounded pagination.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ForkKey {
    /// Immutable relationship keyed by child project ID.
    Relationship(u64),
    /// Parent project ID keyed by child project ID.
    ParentForChild(u64),
    /// Child project IDs for a parent project.
    ChildProjects(u64),
}
