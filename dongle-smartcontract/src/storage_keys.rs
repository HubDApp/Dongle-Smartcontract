...
/// Storage keys for collection visibility, ownership, and share links (#819).
///
/// `StorageKey` and `ExtensionKey` are capped at 50 variants. Collection visibility
/// keys use this independent enum following the same pattern as `BookmarkKey`.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum CollectionVisibilityKey {
    /// Owner address of a collection: collection_id -> Address
    CollectionOwner(u64),
    /// Visibility state of a collection: collection_id -> bool (true = public, false = private)
    CollectionIsPublic(u64),
    /// Active share token for a collection: collection_id -> String
    CollectionShareToken(u64),
    /// Index of collection IDs owned by a user: Address -> Vec<u64>
    UserCollections(Address),
    /// Global index of public collection IDs for discovery: Vec<u64>
